...
/*
	!!! singleton 패턴에서 중요한 것은 다음의 3가지이다. !!!
	== 첫번째,
 ...
 */
	...
	static {
		// 중요한 사실은 static 초기화 블럭은 해당 클래스가 객체로 생성되기 전에 먼저 실행되며
		// 딱 1번만 호출되고, 다음번에 새로운 객체(인스턴스)를 매번 생성하더라도 static 초기화 블럭은 호출되지 않는다.
...
/*
	== 두번째,
	      생성자에 접근제한자를 private으로 지정하여 외부에서 절대로 인스턴스를 생성하지 못하도록 막는다.
 */
	private SingletonNumber() { }  // 기본 생성자, 다섯번째로 호출(작동)
	
/*
	== 세번째,
	   static 메소드를 생성하여 외부에서 해당 클래스의 객체를 사용할 수 있도록 한다.
 */
	public static SingletonNumber getInstance() {  // -- 세번째로 호출(작동)
		return singleton;
	}
	
	public int getNextNumber() {  // 인스턴스 메소드, 여섯번째로 호출(작동)
		return ++cnt;  // 인스턴스 변수
	}
